...
public class ItemWrench extends ItemMeleeWeapon {
	...
	public boolean onHit(ItemStack stack,EntityLivingBase attacker, Entity target){
		...
		if(target instanceof EntityBuilding && TF2weapons.isOnSameTeam((EntityBuilding)target,attacker)){
			...
			if(building.isSapped()){
				...
				if(building instanceof EntitySentry){
					metalUse=(int)Math.min( Math.min(((EntitySentry) building).getMaxAmmo()-((EntitySentry)building).getAmmo(), 40),metalLeft);
					...
					if(building.getLevel()==3){
						metalUse=(int)Math.min( Math.min(20-((EntitySentry)building).getRocketAmmo(), 8),metalLeft/2);
						((EntitySentry)building).setRocketAmmo(Math.min(20,((EntitySentry)building).getRocketAmmo()+metalUse));
						metalLeft-=metalUse*2;
					}
				}
				if(building.getLevel()<3){
					metalUse=(int)Math.min( Math.min(200-building.getProgress(), 25),metalLeft);
				...
				building.playSound(ItemFromData.getSound(stack,metalLeft!=TF2weapons.getMetal(attacker)?PropertyType.BUILD_HIT_SUCCESS_SOUND:PropertyType.BUILD_HIT_FAIL_SOUND), 1.7f, 1f);
