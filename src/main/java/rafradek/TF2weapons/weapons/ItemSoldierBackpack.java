    ...
    public void onArmorTick(World world, final EntityPlayer player, ItemStack itemStack){
    	if(!world.isRemote){
    		if(player.ticksExisted%5==0&&itemStack.getTagCompound().getBoolean("Active")){
    			itemStack.getTagCompound().setFloat("Rage", Math.max(0,itemStack.getTagCompound().getFloat("Rage")-(float)1/(float)(TF2Attribute.getModifier("Buff Duration", itemStack, getData(itemStack).getInt(PropertyType.DURATION),player)-20)));
    			if(itemStack.getTagCompound().getFloat("Rage")<=0){
    				itemStack.getTagCompound().setBoolean("Active", false);
    			}
    			for(EntityLivingBase living : world.getEntitiesWithinAABB(EntityLivingBase.class, player.getEntityBoundingBox().expand(10, 10, 10), new Predicate<EntityLivingBase>(){
