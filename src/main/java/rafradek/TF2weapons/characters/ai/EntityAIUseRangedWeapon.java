...
public class EntityAIUseRangedWeapon extends EntityAIBase {
    ...
    public boolean shouldExecute()
    {
        ...
        else
        {
            ...
            return this.entityHost.getHeldItemMainhand() != null && this.entityHost.getHeldItemMainhand().getItem() instanceof ItemWeapon;
        }
    }
    ...
    public void updateTask()
    {
        ...
        if(this.projSpeed>0){
            ...
            if(this.entityHost.worldObj.rayTraceBlocks(new Vec3d(this.entityHost.posX,this.entityHost.posY+this.entityHost.getEyeHeight(),this.entityHost.posZ), new Vec3d(lookX,lookY,lookZ),false,true,false)!=null){
            ...
            RayTraceResult mop=this.entityHost.worldObj.rayTraceBlocks(this.attackTarget.getPositionVector(), this.attackTarget.getPositionVector().addVector(0, -0.3f-yFall, 0));
        ...
        //System.out.println("raytracing:"+this.entityHost.worldObj.rayTraceBlocks(new Vec3d(this.entityHost.posX,this.entityHost.posY+this.entityHost.getEyeHeight(),this.entityHost.posZ), new Vec3d(lookX,this.attackTarget.posY,lookZ)));
	        if(this.fireAtFeet>0 && this.entityHost.worldObj.rayTraceBlocks(new Vec3d(this.entityHost.posX,this.entityHost.posY+this.entityHost.getEyeHeight(),this.entityHost.posZ), new Vec3d(lookX,this.attackTarget.posY,lookZ),false,true,false)==null){
	        	/*if(this.fireAtFeet==2&&this.attackTarget.motionY<=0){
		        	RayTraceResult mop=this.entityHost.worldObj.rayTraceBlocks(this.attackTarget.getPositionVector(), this.attackTarget.getPositionVector().addVector(0, yFall*1.2, 0));
		        	if(mop != null && mop.typeOfHit==RayTraceResult.Type.BLOCK){
        ...
        if(!this.reloading&&(this.entityHost.worldObj.getDifficulty()!=EnumDifficulty.HARD||!fire)&&item.getItemDamage()==item.getMaxDamage()&&weapon.hasClip(item)){
        	this.reloading=true;
        }
        ...
        if((!reloading/*||(this.entityHost.worldObj.getDifficulty()==EnumDifficulty.HARD&&d0 <= (double)this.attackRangeSquared/4)*/)&&fire && d0 <= (double)this.attackRangeSquared && (this.entityHost.getAmmo() > 0 || weapon.getAmmoType(item)==0)){
        	this.reloading=false;
        	if(!pressed){
        		pressed=true;
        		weapon.startUse(item, this.entityHost, this.entityHost.worldObj, this.entityHost.getCapability(TF2weapons.WEAPONS_CAP, null).state, 1);
        	...
        	if(pressed){
        		...
        		weapon.endUse(item, this.entityHost, this.entityHost.worldObj, this.entityHost.getCapability(TF2weapons.WEAPONS_CAP, null).state, valuedef);
        ...
        //if(){
        	...
        	if(this.dodge&&this.entityHost.getNavigator().noPath()){
	        	Vec3d Vec3d = RandomPositionGenerator.findRandomTarget((EntityCreature) this.entityHost, 4, 2);
	            ...
	            if (Vec3d != null)
	            {
	                ...
	                this.entityHost.getNavigator().tryMoveToXYZ(Vec3d.xCoord+offsetX, Vec3d.yCoord+offsetY, Vec3d.zCoord+offsetZ, this.entityMoveSpeed*this.dodgeSpeed);
	            }
        	}
       // }
    }
