...
public class EntityAINearestChecked extends EntityAITarget {
            ...
            public boolean apply(EntityLivingBase target)
            {
                ...
                else
                {
                    ...
                    if (target instanceof EntityLivingBase)
                    {
                        ...
                        if(target.hasCapability(TF2weapons.WEAPONS_CAP, null)&&(target.getCapability(TF2weapons.WEAPONS_CAP, null).invisTicks>=20||(ItemDisguiseKit.isDisguised(target)&&(target.getCapability(TF2weapons.WEAPONS_CAP, null).invisTicks==0||taskOwner instanceof EntityBuilding)))){
                        	d0=taskOwner instanceof EntityBuilding?0:1;
                        }
                        boolean fastCheck=taskOwner instanceof EntityBuilding ||(!(target instanceof EntityPlayer)&&(TF2weapons.naturalCheck.equals("Fast")&&taskOwner instanceof EntityTF2Character && ((EntityTF2Character)taskOwner).natural));
                        if ((double)target.getDistanceToEntity(EntityAINearestChecked.this.taskOwner) > d0||(!fastCheck&&!TF2weapons.lookingAt(EntityAINearestChecked.this.taskOwner, 105, target.posX, target.posY+target.getEyeHeight(), target.posZ)))
                        {
                            return false;
                        }
	...
	public boolean shouldExecute()
    {
        ...
        if(((this.taskOwner.getAttackTarget()==null)&&this.targetChoosen>1)||this.targetChoosen>5){
            ...
            List<? extends EntityLivingBase> list = this.taskOwner.worldObj.getEntitiesWithinAABB(this.targetClass, this.taskOwner.getEntityBoundingBox().expand(d1, d0, d1), this.targetEntitySelector);
    ...
    protected boolean isSuitableTarget(EntityLivingBase target, boolean includeInvincibles)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    if (target instanceof IEntityOwnable && ((IEntityOwnable)this.taskOwner).getOwnerId().equals(((IEntityOwnable)target).getOwnerId()))
                    {
                        return false;
                    }
