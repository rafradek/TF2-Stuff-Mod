...
public class RenderTF2Character extends RenderBiped<EntityTF2Character> {
    ...
    {
		String clazz = null;
		if(par1EntityLiving.getDataManager().get(TF2EventBusListener.ENTITY_DISGUISED)&&par1EntityLiving.getDataManager().get(TF2EventBusListener.ENTITY_DISGUISE_TYPE).startsWith("T:")){
			clazz=par1EntityLiving.getDataManager().get(TF2EventBusListener.ENTITY_DISGUISE_TYPE).substring(2);
		}
		else{
		...
		if(par1EntityLiving.getEntTeam()==0||(par1EntityLiving.getEntTeam()==1&&par1EntityLiving.getDataManager().get(TF2EventBusListener.ENTITY_DISGUISED))){
			switch(clazz){
	...
	public void doRender(EntityTF2Character living, double p_76986_2_, double p_76986_4_, double p_76986_6_, float p_76986_8_, float p_76986_9_)
    {
		...
		if(living.getHeldItemMainhand()!=null && !(living.getHeldItemMainhand().getItem() instanceof ItemMeleeWeapon)){
			...
			this.modelBipedMain.rightArmPose=((living.getCapability(TF2weapons.WEAPONS_CAP, null).state&3)>0)||sniperZoomed?ModelBiped.ArmPose.BOW_AND_ARROW:ModelBiped.ArmPose.EMPTY;
		}
		...
		if(living.isEntityAlive()){
			...
			if(sniperZoomed){
		        ...
		        GlStateManager.translate((float)p_76986_2_, (float)p_76986_4_+living.getEyeHeight(), (float)p_76986_6_);
