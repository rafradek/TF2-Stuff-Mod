...
public class TileEntityAmmoFurnace extends TileEntityLockable implements ITickable, ISidedInventory
{
    ...
    public void update()
    {
        ...
        {
            if (this.isBurning() || this.furnaceItemStacks[9] != null)
            {
                if (!this.isBurning() && this.canSmelt())
                {
                    this.furnaceBurnTime = getItemBurnTime(this.furnaceItemStacks[9]);
                    ...
                    if (this.isBurning())
                    {
                        ...
                        if (this.furnaceItemStacks[9] != null)
                        {
                            ...
                            {
                                this.furnaceItemStacks[9] = furnaceItemStacks[9].getItem().getContainerItem(furnaceItemStacks[9]);
                            }
                        }
                    }
                }
            ...
            {
                flag1 = true;
                BlockAmmoFurnace.setState(this.isBurning(), this.worldObj, this.pos);
            }
        }
    ...
    private boolean canSmelt()
    {
    	...
    	for(int i=0;i<9;i++){
    		ItemStack base=this.furnaceItemStacks[i];
    		//System.out.println("Base: "+i+" "+base+" "+ItemAmmo.AMMO_RECIPES[base.getItemDamage()]);
    		if(base != null && base.getItem() instanceof ItemAmmo && TF2CraftingManager.AMMO_RECIPES[base.getItemDamage()] != null){
    			/*&& base.stackSize>=ItemAmmo.AMMO_RECIPES[base.getItemDamage()].getRecipeOutput().stackSize){*/
    			ammoTypesCount[base.getItemDamage()]+=base.stackSize;
    			if(ammoTypesCount[base.getItemDamage()]>=MathHelper.ceiling_float_int(TF2CraftingManager.AMMO_RECIPES[base.getItemDamage()].getRecipeOutput().stackSize*1.2f)){
    ...
    {
    	int ammoToConsume=MathHelper.ceiling_float_int(TF2CraftingManager.AMMO_RECIPES[this.ammoSmeltType].getRecipeOutput().stackSize*1.2f);
    	for(int i=0;i<9;i++){
    		ItemStack base=this.furnaceItemStacks[i];
    		if(base != null && base.getItem() instanceof ItemAmmo && base.getItemDamage()==this.ammoSmeltType){
    			...
    			int ammoConsumed=Math.min(base.stackSize, ammoToConsume);
    			base.stackSize-=ammoConsumed;
    				...
    				for(Object obj:recipe.getInput()){
        				...
        				for(int j=10;j<19;j++){
        					...
        					if(inSlot==null){
        						...
        						int size=out.stackSize+inSlot.stackSize;
